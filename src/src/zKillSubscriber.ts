...
export class ZKillSubscriber {
          ...
          try {
            ...
            switch (subscription.subType) {
              ...
              case SubscriptionType.ALLIANCE:
                ...
                if (requireSend) {
                  if (
                    subscription.limitType !== LimitType.NONE &&
                    !(await this.isInLimit(subscription, data.solar_system_id))
                  ) {
                    return;
                  }
                  await this.sendKill(
    ...
    await this.asyncLock.acquire("sendKill", async (done) => {
      ...
      if (c) {
        ...
        try {
          ...
          // @ts-ignore
          const finalBlow = embedding?.result.ogDescription.match(
            /(?<=Final Blow by).*(?=\()/
          );
          const totalValue = data.zkb.totalValue;
          const totalValueString = totalValue
            .toString()
            .replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
          totalValueString.replace(/(.)(?=(\d{3})+$)/g, "$1.");
          description = `Final blow: ${finalBlow} \n Total value: ${totalValueString} ISK`;
        } catch (e) {
          // Do nothing
        }
        try {
  ...
  public static getInstance(client?: Client) {
    if (!this.instance && client) this.instance = new ZKillSubscriber(client);
      ...
      if (file.name.match(/\d+\.json$/)) {
        ...
        if (guildId && guildId.length > 0 && guildId[0]) {
          ...
          this.subscriptions.set(guildId[1], {
            channels: this.createChannelMap(parsedFileContent.channels),
          });
        }
      }
    }
  ...
  private createChannelMap(object: any): Map<string, SubscriptionChannel> {
    ...
    for (const key of keys) {
      map.set(key, {
        subscriptions: this.createSubscriptionMap(object[key].subscriptions),
      });
    }
    return map;
  }
